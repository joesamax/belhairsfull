...
function xtfw_newest_fw_plugin_first() {
	...
	/**
	 * @todo Multi-site network activated plugin are always loaded prior to site plugins so if there's a a plugin activated in the network mode that has an older version of the SDK of another plugin which is site activated that has new XT Framework version, the essential-functions.php will be loaded from the older SDK. Same thing about MU plugins (loaded even before network activated plugins).
	 *
	 * @link https://github.com/Freemius/wordpress-fw/issues/26
	 */

	$newest_fw_plugin_path = $xtfw_active_plugins->newest->plugin_path;

	$active_plugins       = get_option( 'active_plugins', array() );
	$newest_fw_plugin_key = array_search( $newest_fw_plugin_path, $active_plugins );
	if ( 0 === $newest_fw_plugin_key ) {
		// if it's 0 it's the first plugin already, no need to continue
		return false;
	} else if ( is_numeric( $newest_fw_plugin_key ) ) {
		// Remove plugin from its current position.
		array_splice( $active_plugins, $newest_fw_plugin_key, 1 );

		// Set it to be included first.
		array_unshift( $active_plugins, $newest_fw_plugin_path );

		update_option( 'active_plugins', $active_plugins );

		return true;
	} else if ( is_multisite() && false === $newest_fw_plugin_key ) {
...
function xtfw_print_queued_js() {
	...
	if ( ! empty( $xtfw_queued_js ) ) {
		 ...
		 * @since 21.0.0
		 */
		echo apply_filters( 'xtfw_queued_js', $js ); // WPCS: XSS ok.
	}
}
