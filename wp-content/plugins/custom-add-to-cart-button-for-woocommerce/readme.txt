=== Ultimate Custom Add To Cart Button For WooCommerce ===
Contributors: bc2018
Tags: woocommerce, add to cart, add to cart button, custom add to cart button
Donate link: https://www.binarycarpenter.com/ultimate-add-to-cart-button-for-woocommerce/
Requires at least: 4.7
Tested up to: 5.3.2
Requires PHP: 5.3
License: GPL 2.0
License URI: https://www.gnu.org/licenses/gpl-2.0.en.html

Ultimate *Custom Add To Cart Button* For WooCommerce let you fully customize the add to cart button on your WooCommerce store. Your WoComerce add to cart button will attract a lot of sales.

== Description ==
## Why do you need a Custom WooCommerce Add To Cart Button Plugin?
Add to cart button is one of the most important element of any store. However, WooCommerce allows little customization by default to this element. A well designed add to cart button can help you sky-rocketed your sales.
Thus, Ultimate Custom Add To Cart Button For WooCommerce is the tool to help you create world-class button to boost your sales through the roof.


## Ultimate Add To Cart Buttons Plugin For WooCommerce In Action
[youtube https://www.youtube.com/watch?v=ZWvypChzFwo]

## What are the features of Ultimate Custom Add To Cart Button For WooComerce
* Customize button text on simple products
* Customize button text on variable products
* Customize button text on grouped product
* Change button's background color
* Customize your button's border radius
* Completely change the style of the quantity box, remove the arrows on number input, add plus and minus buttons to the left and right of the quantity input box
* Change button's on hover background color
* Use gradient as your add to cart button background [Pro version only](https://binarycarpenter.com/ultimate-add-to-cart-button-for-woocommerce/)
* Use gradient as your button hover state [Pro version only](https://binarycarpenter.com/ultimate-add-to-cart-button-for-woocommerce/)
* Use image as add to cart button [Pro version only](https://binarycarpenter.com/ultimate-add-to-cart-button-for-woocommerce/)
* Insert add and minus icon around the quantity box [Pro version only](https://binarycarpenter.com/ultimate-add-to-cart-button-for-woocommerce/)
* Create your own styles [Pro version only, coming soon](https://binarycarpenter.com/ultimate-add-to-cart-button-for-woocommerce/)
* Customize button's size (width, height...)
* Add cart icons to your add to cart button
* Works with all themes
* Comes with pre-built add to cart button from Amazon, Lazada and shopee


I write about [WooCommerce & WordPress Tutorials](https://www.binarycarpenter.com/) here.

### Check out some of the most popular posts on my blog
* [WooCommerce Product Table](https://wordpress.org/plugins/free-product-table-for-woocommerce/)


### Checkout my other free plugins
* [WooCommerce Product Table](https://wordpress.org/plugins/free-product-table-for-woocommerce/)
* [WooCommerce Menu Cart Icon](https://wordpress.org/plugins/bc-menu-cart-woo/)
* [WooCommerce Customize Add To Cart Buttons](https://wordpress.org/plugins/custom-add-to-cart-button-for-woocommerce/)
* [LaunchPage.app Importer](https://wordpress.org/plugins/launchpage-app-importer/)
* [Free Landing Page - Squeeze Page Builder](https://wordpress.org/plugins/free-sales-funnel-squeeze-pages-landing-page-builder-templates-make/)
* [WooSliderPro Drag and drop slider for WooCommerce](https://wordpress.org/plugins/woo-slider-pro-drag-drop-slider-builder-for-woocommerce/)


### Checkout some great posts on my website
* [WooCommerce composite products](https://www.binarycarpenter.com/create-woocommerce-composite-products/)
* [Remove WooCommerce breadcrumbs](https://www.binarycarpenter.com/easy-ways-to-remove-woocommerce-breadcrumbs/)
* [WooCommerce Force Sells](https://www.binarycarpenter.com/woocommerce-force-sell-in-5-minutes/)
* [Create WooCommerce tickets](https://www.binarycarpenter.com/woocommerce-tickets-create-sell-in-5-minutes/)
* [Customize WooCommerce Related Products](https://www.binarycarpenter.com/1-plugin-to-customize-woocommerce-related-products/)
* [WooCommerce Product Shortcodes](https://www.binarycarpenter.com/the-ultimate-guide-to-woocommerce-product-shortcodes/)
* [WooCommerce not sending emails fix](https://www.binarycarpenter.com/the-ultimate-fix-to-woocommerce-not-sending-emails-problem-for-free/)
* [WooCommerce dynamic pricing](https://www.binarycarpenter.com/woocommerce-dynamic-product-pricing-discount-the-ultimate-guide/)
* [Woocommerce table rate shipping plugin](https://www.binarycarpenter.com/ultimate-woocommerce-table-rate-shipping-tutorial-use-cases/)
* [WooCommerce grouped products tutorials](https://www.binarycarpenter.com/the-ultimate-guide-to-woocommerce-grouped-products/)
* [Woocommerce flat rate shipping tutorial](https://www.binarycarpenter.com/the-ultimate-guide-to-woocommerce-flat-rate-shipping/)
* [How to customize Woocommerce shop page](https://www.binarycarpenter.com/how-to-customize-woocommerce-shop-page-in-5-minutes/)
* [WooCommerce shipping classes tutorial](https://www.binarycarpenter.com/the-ultimate-guide-to-woocommerce-shipping-classes/)
* [Search multiple excel files software](https://www.binarycarpenter.com/excel-software-how-to-search-for-text-through-multiple-big-excel-files/)


== Screenshots ==
1. Use cart icon, loading icon and add icon on add to cart button
2. Use image for cart icon, loading icon and added to cart cart icon

== Installation ==
*	Upload the extracted folder of the plugin to `/wp-content/plugins/` directory
*	Activate the plugin in your WordPress dashboard


== Change Log ==
2019-01-19: First upload
