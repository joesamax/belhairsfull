...
function wcz_wc_texts()
{
    ...
    if ( is_woocommerce() || is_cart() ) {
        ...
        // ---------------- THIS IS BREAKING VARIABLE PRODUCT PAGES ON SSOME SITES
    ...
    if ( get_option( 'wcz-add-shop-button', woocustomizer_library_get_default( 'wcz-add-shop-button' ) ) ) {
        add_action( 'woocommerce_after_add_to_cart_form', 'wcz_add_product_shopping_button', 31 );
    }
    // Remove Product SKU
...
function wcz_stock_availability_text( $availability )
{
    ...
    if ( 'outofstock' == $product->get_stock_status() ) {
        $wcz_oostext = get_option( 'wcz-product-outofstock-txt', woocustomizer_library_get_default( 'wcz-product-outofstock-txt' ) );
        $availability['availability'] = str_ireplace( 'Out of stock', $wcz_oostext, $availability['availability'] );
    }
    ...
    if ( get_option( 'wcz-show-other-stockstatus', woocustomizer_library_get_default( 'wcz-show-other-stockstatus' ) ) ) {
        ...
        if ( 'onbackorder' == $product->get_stock_status() ) {
            $wcz_obotext = get_option( 'wcz-product-onbackorder-txt', woocustomizer_library_get_default( 'wcz-product-onbackorder-txt' ) );
            $availability['availability'] = str_ireplace( 'On Back Order', $wcz_obotext, $availability['availability'] );
        }
        
        // Change In Stock Text
        
        if ( 'instock' == $product->get_stock_status() ) {
            $wcz_istext = get_option( 'wcz-product-instock-txt', woocustomizer_library_get_default( 'wcz-product-instock-txt' ) );
            $availability['availability'] = str_ireplace( 'Stock Available', $wcz_istext, $availability['availability'] );
        }
...
{
    echo  '<a class="button wcz-continue" href="' . esc_url( get_permalink( wc_get_page_id( 'shop' ) ) ) . '">' . get_option( 'wcz-add-shop-button-txt', woocustomizer_library_get_default( 'wcz-add-shop-button-txt' ) ) . '</a>' ;
}
...
function wcz_wc_extras()
{
    ...
    // Rename Account Page Titles/Endpoints
    if ( is_product() && (get_option( 'wcz-add-price-prefix', woocustomizer_library_get_default( 'wcz-add-price-prefix' ) ) || get_post_meta( get_the_ID(), 'wcz_pps_price_prefix', true )) ) {
        add_filter(
            'woocommerce_get_price_html',
            'wcz_add_price_prefix',
            99,
            2
        );
    }
    if ( is_product() && (get_option( 'wcz-add-price-suffix', woocustomizer_library_get_default( 'wcz-add-price-suffix' ) ) || get_post_meta( get_the_ID(), 'wcz_pps_price_prefix', true )) ) {
        add_filter(
...
{
    $wcz_price_suffix = ( get_post_meta( get_the_ID(), 'wcz_pps_price_suffix', true ) ? get_post_meta( get_the_ID(), 'wcz_pps_price_suffix', true ) : get_option( 'wcz-add-price-suffix-txt', woocustomizer_library_get_default( 'wcz-add-price-suffix-txt' ) ) );
    $html .= ' <small>' . esc_html( $wcz_price_suffix ) . '</small>';
    return $html;
}

function wcz_add_price_prefix( $price, $product )
{
    $wcz_price_prefix = ( get_post_meta( get_the_ID(), 'wcz_pps_price_prefix', true ) ? get_post_meta( get_the_ID(), 'wcz_pps_price_prefix', true ) : get_option( 'wcz-add-price-prefix-txt', woocustomizer_library_get_default( 'wcz-add-price-prefix-txt' ) ) );
    $price = '<small>' . esc_html( $wcz_price_prefix ) . '</small> ' . $price;
    return $price;
}
