...
! defined( 'WOOBT_VERSION' ) && define( 'WOOBT_VERSION', '3.2.1' );
			...
			class WPCleverWoobt {
				...
				function woobt_individually_found_in_cart( $found_in_cart, $product_id ) {
					if ( $this->woobt_check_in_cart( $product_id ) ) {
						// removed get_post_meta( $product_id, 'woobt_ids', true )
						return true;
					}
							...
							if ( $items = $this->woobt_get_items( $_POST['woobt_ids'] ) ) {
								foreach ( $items as $item ) {
									...
									if ( $item_product->is_sold_individually() && $this->woobt_check_in_cart( $item['id'] ) ) {
										wc_add_notice( sprintf( esc_html__( 'You cannot add another "%s" to the cart.', 'woobt' ), esc_html( $item_product->get_name() ) ), 'error' );
										wc_add_notice( esc_html__( 'You cannot add this product to the cart.', 'woobt' ), 'error' );
